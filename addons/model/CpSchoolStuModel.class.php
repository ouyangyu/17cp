...
class CpSchoolStuModel extends Model {
    ...
    public function getStudentList($schoolInfoId, $limit = 10,$order = "student_id") {
        ...
        if(!empty($schoolInfoId)){
            ...
            $list = $this->where($map)->order($order)->page($nowPage.','.$limit)->select();
    ...
    public function importStudentInfo($user) {
        if(is_array($user)) {
            // 添加昵称拼音索引
            $user ['first_letter'] = getFirstLetter ( $user ['student_name'] );
            // 如果包含中文将中文翻译成拼音
